...
# nicely merge and reorganize tables to look like Ying's
# compare TMLE results to non-TMLE

# write the relevant R objects for use in manuscript
setwd(results.dir)
...
########################### JUST FOR FUN (POST HOC): META-ANALYZE THE PRIMARY RESULTS ###########################
...
# which variables are rare?
setwd(results.dir)
rares = read.csv("list_of_rare_binaries.csv")$name
...
xlab = "Estimated OR (becoming married vs. not) and E-values (RR)"
if ( analysis.dir.name == "Divorce" ) xlab = "Estimated odds ratio (becoming divorced vs. not) and E-values (RR)"
...
ggplot( data = d, aes( x = exp(yi), y = Outcome, color = pass.bonferroni ) ) +
  ...
  # E-values
  geom_point( aes( x = Evalue.point, y = Outcome, shape = "shape1" ), size = 2, color = "red" ) +
  geom_point( aes( x = Evalue.CI, y = Outcome, shape = "shape2" ), size = 2, color = "red" ) +
  
  #scale_x_continuous( breaks = seq(.9, 1.8, .05)) +
  
  scale_shape_manual( values = c("shape1" = 8,
                                 "shape2" = 4),
                      name = "E-values",
                      guide = "legend",
...
# 
# # and merge OLS and logistic results for each type of analysis
# 
# # codebook contains the intended variable ordering
# require(readxl)
# cd = read_xlsx("Analysis dataset codebook.xlsx")
# 
# # glue the results for OLS and logistic
# setwd(results.dir)
# setwd("Marriage")
# res = rbind( read.csv("table2_logistic_nontmle_MI_.csv"),
#            read.csv("table2_OLS_nontmle_MI_.csv") )
# 
# # reorder them
# res$order = NA
# 
# 
# require(plyr)
# table2$order = mapvalues( res$Outcome, from = cd$`Long name`, to = cd$`Table order`)
# 
# require(plyr)
