...
results.dir = paste(root.dir, "[PRIVATE] Data and results/Results/Divorce", sep="/")
stochastic.results.dir = paste(root.dir, "[PRIVATE] Data and results/Results/Divorce/R objects from analysis", sep="/")
codebook.dir = paste(root.dir, "[PRIVATE] Data and results/Data", sep="/")

# # COMMENTED OUT FOR SPEED
# library(sas7bdat)
# setwd(data.dir)
# d = read.sas7bdat( "divorce_full_data.sas7bdat" )
# nrow(d)  # 89,897
# # save it to avoid long read-in process
# write.csv( d, "divorce_full_data.csv", row.names = FALSE )

setwd(data.dir)
library(readr)
d = suppressMessages( read_csv("divorce_full_data.csv") )

library(testthat)
expect_equal( nrow(d), 89897) # compare to Ying's table 2
...
( m = lm( y.centered ~ mars93_3 + age + nhwhite + colled + nim89 + region2 + region3 + region4 + mdinc2 + mdinc3 + mdinc4 + exam89 + alco89 + smoke89 + act89_d + number, data = d ) )
summary(m)
...
# this is to help anticipate problems with mice() due to collinearity
...
ols.tmle.time = end.time - start.time

# per outcome
ols.tmle.time / ( length(Ylin) * M )
...
logistic.tmle.time = end.time - start.time

# per outcome
logistic.tmle.time / ( length(Ybin) * M )
